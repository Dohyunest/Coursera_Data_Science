...
-   Big Data란 기존 Database 관리도구의 능력을 넘어서는 대량의 정형 또는
    비정형 데이터 집합조차 포함한 데이터로부터 가치를 추출하고 결과를
    분석하는 기술이다. 일반적으로 빅데이터는 3V로 정의되며 Volume
    (데이터의 양), Velocity (데이터 생성 속도), Variety (형태의
    다양성)와 같다.

-   정형 데이터란 미래 정해 놓은 형식과 구조에 따라 저장되도록 구성하여
    고정된 필드에 저장된 데이터이다. 대표적으로 열과 행이 있는 테이블,
    스프레드시트, 데이터베이스가 있다.

-   비정형 데이터란 정의된 구조가 없는 동영상 파일, 오디오 파일, 사진,
    보고서, 메일 본문 등과 같이 정형화되지 않은 데이터이다.

-   반전형 데이터란 데이터의 구조 정보를 함께 제공하는 파일 형식의
    데이터로, 데이터의 형식과 구조가 변경될 수 있는 데이터입니다.
    대표적으로 XML, HTML, JSON 등이 있다.

-   빅데이터 작업의 이점

    -   데이터에 약간의 지저분함이나 부정확성이 있는 경우 데이터의
        방대한 양은 이러한 작은 오류의 영향을 무효화시킨다.  
    -   지속적으로 업데이트되는 데이터가 있는 경우 분석이 어려울 수
        있지만 실시간 최신 정보를 보유할 수 있으므로 현재 상태에 대한
        정확한 분석을 수행할 수 있다.  
    -   정보 부족으로 이전에 대답할 수 없었던 질문에 대해 새로운 연결 및
        발견이 이루어질 수 있다.
    -   단, 빅데이터라고 해도 올바른 데이터가 아닌 경우 이를 활용할
        방법은 없으므로 데이터의 양 뿐만 아니라 퀄리티도 굉장히
        중요하다.
